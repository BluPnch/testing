...
using ContinuousMetricsService.net8.Helpers;
using ContinuousMetricsService.net8.Models;


namespace ContinuousMetricsService.net8.Services;
...
public class ContinuousMetricsService : BackgroundService
{
    ...
    private async Task RunCountByBenchmarks()
    {
        await Task.Run(() =>
        {
            ...
            // CountBy_Net9 недоступен в NET8
        });
    }
    ...
    private async Task RunAggregateByBenchmarks()
    {
        await Task.Run(() =>
        {
            ...
            RunAndRecordMetrics("AggregateBy_Select", () => AggregateBy_Select(_testData));
            // AggregateBy_Net9 недоступен в NET8
        });
    }
    ...
    private Dictionary<string, decimal> AggregateBy_Select(TestData[] data)
    {
        var countingData = new CountingEnumerable<TestData>(data);
        return countingData
            .GroupBy(x => x.Category)
