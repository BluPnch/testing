...
namespace UnitTests.TestRepositories
{
    ...
    public class TestJournalRecordRepository : IClassFixture<RepositoryTestFixture>, IDisposable
    {
        ...
        public async Task AddJournalRecordAsync_ShouldAddRecordToDatabase()
        {
            ...
            // Создание записи журнала с зависимостями
            AllureApi.Step("Create journal record with dependencies", () => {
        ...
        public async Task GetAllJournalRecordsAsync_ShouldReturnAllRecords()
        {
            ...
            AllureApi.Step("Verify 2 records returned", () => {
        ...
        public async Task GetJournalRecordByIdAsync_ShouldReturnRecord_WhenExists()
        {
            ...
            AllureApi.Step("Verify record data", () => {
        ...
        public async Task UpdateJournalRecordAsync_ShouldUpdateRecord()
        {
            var (plantId, growthStageId, employeeId) = await SetupDependenciesAsync();
            var recordId = Guid.NewGuid();
            JournalRecord updatedRecord = null!;
            ...
            AllureApi.Step("Create updated journal record", () => {
                updatedRecord = new JournalRecordBuilder()
                    .WithId(recordId)
                    .WithPlantId(plantId)
                    .WithGrowthStageId(growthStageId)
                    .WithEmployeeId(employeeId)
        ...
        {
            JournalRecord nonExistentRecord = null!;

            AllureApi.Step("Create non-existent journal record", () => {
                nonExistentRecord = new JournalRecordBuilder().Build();
            });
